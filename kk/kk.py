...
def show_help_message():
    ...
    print(
        "#{} (0{} for example): show numbers that are still needed in a row".format(
            ROW_CMD, ROW_CMD
        )
    )
    print(
        "#{} (0{} for example): show numbers that are still needed in a col".format(
            COL_CMD, COL_CMD
        )
    )
    ...
    print(
        "{}: play again, re-entering your board size and difficulty".format(REPLAY_CMD)
    )
    print(
        "{}: show cages (useful when adjacent cages share same values)".format(
            CAGES_CMD
        )
    )
    print(
        BAD_COLOR
        + "highlight color when the same number occurs more than once in a row/col"
        + ENDC
    )
    print(BAD_CAGE_COLOR + "highlight color when a cage's math is incorrect" + ENDC)
    print("************************************")
        ...
        for i in range(len(self.b)):
            ...
            for z in range(len(self.b[i])):
                ...
                if self.color:
                    ...
                    if self.mistakes:
                        if guy != PLACEHOLDER and (
                            self.b[i].count(int(guy)) > 1
                            or self.inverse[z].count(int(guy)) > 1
                        ):
                            the_color = BAD_COLOR
                    if self.cages:
...
def build_cage_board(cages, n):
    ...
    for row in range(len(cage_board)):
        for col in range(len(cage_board[row])):
            cage_board[row][col] = (
                cage_board[row][col]
                + (" " * (max_res + len(COLORS[0]) - len(cage_board[row][col])))
                + ENDC
            )
...
def build_b(n):
    ...
    board[0] = list(random.sample(list(choices), 1)[0])
    ...
    while not is_valid:
        ...
        candidate = random.sample(list(choices), 1)[0]
...
def build_c(board, d):
    ...
    # ops = ['*']*d + ['+']*d + ['-']*d + ['/']*d + ['.']
    ops = ("*", "+", "-", "/")
    # TODO: consider alternate strategy for determining max_cage_size
            ...
            for c in cage:
                ...
                if row_diff + col_diff == 1:
                    if op == "/" and board[c[0]][c[1]] != board[choice[0]][choice[1]]:
                        division = max(
                            board[c[0]][c[1]], board[choice[0]][choice[1]]
                        ) / (min(board[c[0]][c[1]], board[choice[0]][choice[1]]) * 1.0)
...
def init_cages(board, difficulty):
    ...
    while has_ambiguous_cages(cages):
        cages = build_c(board, difficulty)
    cage_board, color_board = build_cage_board(cages, len(board))
    ...
    if not ai_size:
        while not done:
            try:
                ...
                size = int(size)
                diff = int(input("enter difficulty between 1 and " + str(size) + ": "))
                done = True
            except ValueError:
        ...
        for potential in remaining_ans_for_row_or_col:
            ...
            for other_row_or_col in other_rows_or_cols:
                for r in range(len(userboard)):
                    guy = (
                        userboard[r][other_row_or_col]
                        if userboard != userboard_or_inverse
                        else userboard[other_row_or_col][r]
                    )
...
def guess(userboard, cages):
    ...
    for i in range(len(userboard)):
        ...
        for j in range(len(userboard)):
            ...
            row_col_intersect = set(remaining_ans_for_col).intersection(
                set(remaining_ans_for_row)
            )
        ...
        if cages[cage][1] != ".":
            ...
            if len(current_calculation_for_cage) == size_of_cage - 1:
                ...
                for potential_answer in potential_answers:
                    if op == "/":
                        cage_result = max(
                            potential_answer, current_calculation_for_cage[0]
                        ) / (
                            min(potential_answer, current_calculation_for_cage[0]) * 1.0
                        )
                    else:
                ...
                for correct_answer in copy.deepcopy(correct_answers):
                    if (
                        correct_answer in userboard[cell_in_cage_to_finish[0]]
                        or correct_answer in inverse[cell_in_cage_to_finish[1]]
                    ):
                        correct_answers.remove(correct_answer)

                if len(correct_answers) == 1:
                    print(
                        "all cells in cage filled except {},{} needs {}".format(
                            cell_in_cage_to_finish[0],
                            cell_in_cage_to_finish[1],
                            correct_answers[0],
                        )
                    )
                    return (
...
def main(ai_size=None, ai_diff=None):
    ...
    while not is_win(userboard, board):
        ...
        print(
            ListBoard(
                b=userboard, normal=True, color=color_board, cages=cages, mistakes=True
            )
        )
        try:
            if INTERACTIVE:
                vals = input("enter value as `row,col,val`: ").split(",")
            else:
                print("making move:")
            ...
            if vals[0].lower() == CAGES_CMD:
                ...
                if vals[0][0].isdigit() and vals[0][1].lower() == COL_CMD:
                    ...
                    for i in range(len(inverse)):
                        for j in range(len(inverse)):
                            inverse[j][i] = userboard[i][j]
                    remaining_ans_for_col = set(playable_numbers).difference(
                        set(inverse[int(vals[0][0])])
                    )
                    print(
                        "Numbers remaining in column {}: {}".format(
                            vals[0][0], sorted(list(remaining_ans_for_col))
                        )
                    )
                elif vals[0][0].isdigit() and vals[0][1].lower() == ROW_CMD:
                    playable_numbers = list(range(1, len(userboard) + 1, 1))
                    remaining_ans_for_row = set(playable_numbers).difference(
                        set(userboard[int(vals[0][0])])
                    )
                    print(
                        "Numbers remaining in row {}: {}".format(
                            vals[0][0], sorted(list(remaining_ans_for_row))
                        )
                    )
            else:
    ...
    print(
        "it took you {}:{} to beat this {}x{} board (difficulty {})".format(
    ...
    if INTERACTIVE:
        ...
        if not SAVE:
            again = input(
                "do you want to save this board?: 'y' for yes, anything else no: "
            )
        if again == "y":
            save_playthrough_to_file(board, cages, d, total_time)

        again = input("want to play again? 'a' for again, anything else to quit: ")
...
def test_time(board_size, difficulty):
    print(
        "testing time it takes to generate a board of size {} with difficulty {}".format(
...
def test_difficulty(board_size=5, boards_to_test=100):
    ...
    print(
        "testing average number of squares for a board of size {} with max difficulty {} and number of boards to test {}".format(
            board_size, max_difficulty_to_check, boards_to_test
        )
    )
    ...
    for diff in range(1, max_difficulty_to_check + 1, 1):
        ...
        print(
            "average # of filled in squares for difficulty {}: {} or {}%".format(
                diff, float(s) / boards_to_test, percent_filled
            )
        )
