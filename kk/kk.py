...
def build_cb(cages, n):
    ...
    for row in range(len(cage_board)):
        for col in range(len(cage_board[row])):
            cage_board[row][col] = cage_board[row][col] + \
                (' ' * (max_res + len(COLORS[0]) - len(cage_board[row][col]))) + ENDC
            ...
            for c in cage:
                ...
                if row_diff + col_diff == 1:
                    if op == '/' and board[c[0]][c[1]] != board[choice[0]][choice[1]]:
                        division = max(board[c[0]][c[1]], board[choice[0]][choice[1]]) / \
        ...
        if cages[cage][1] != '.':
            ...
            if len(current_calculation_for_cage) == size_of_cage - 1:
                ...
                for potential_answer in potential_answers:
                    if op == '/':
                        cage_result = max(potential_answer, current_calculation_for_cage[0]) / (
                ...
                if len(correct_answers) == 1:
                    print('all cells in cage filled except {},{} needs {}'.format(
                        cell_in_cage_to_finish[0], cell_in_cage_to_finish[1], correct_answers[0]))
...
def main(ai_size=None, ai_diff=None):
        ...
        try:
            ...
            if vals[0].lower() == CAGES_CMD:
                ...
                if vals[0][0].isdigit() and vals[0][1].lower() == COL_CMD:
                    ...
                    print('Numbers remaining in column {}: {}'.format(
                    ...
                    print('Numbers remaining in row {}: {}'.format(
    ...
    if INTERACTIVE:
        again = 'y'
        if not SAVE:
            again = input('do you want to save this board?: \'y\' for yes, anything else no: ')
        if again == 'y':
...
def test_time(board_size, difficulty):
    print('testing time it takes to generate a board of size {} with difficulty {}'.format(
...
def test_difficulty(board_size=5, boards_to_test=100):
    ...
    print('testing average number of squares for a board of size {} with max difficulty {} and number of boards to test {}'.format(
    ...
    for diff in range(1, max_difficulty_to_check + 1, 1):
        ...
        print('average # of filled in squares for difficulty {}: {} or {}%'.format(
